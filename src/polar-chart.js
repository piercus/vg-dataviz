...
Plotly.d3.csv('./data.csv', (err, rows) => {
	...
	const data = names.map(name => {
		...
		return {
			...
			theta: unpack(filtered, r => {
				let res = (Number.parseInt(r.heading, 10) - Number.parseInt(r.wind10mDirection, 10) + 360) % 360;
				if (res > 180) {
					res = 360 - res;
				}
			...
			marker: {color: unpack(filtered, r => {
				const selectedColor = colors.find(c => c.max === null || c.max > Number.parseInt(r.wind10mSpeed));
	...
	const myPlot = document.querySelector('#myDiv');
